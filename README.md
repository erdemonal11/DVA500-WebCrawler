...
[![Release](https://github.com/erdemonal11/DVA500-WebCrawler/actions/workflows/release.yml/badge.svg)](https://github.com/erdemonal11/DVA500-WebCrawler/actions/workflows/release.yml)
...
Static code analysis is configured using [Ruff](https://docs.astral.sh/ruff/). Configuration is in `ruff.toml`.
...
Pre-commit hooks automatically run static analysis before each commit. Configuration is in `.pre-commit-config.yaml`.
...
Unit tests are written with [pytest](https://docs.pytest.org/). Tests are located in `tests/`.
